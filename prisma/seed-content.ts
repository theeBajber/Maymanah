...
async function main() {
  ...
  // Enroll student in remaining courses
  const student = await prisma.user.findUnique({ where: { email: 'student@maymanah.com' } });
  if (student) {
    ...
    for (const c of allCourses) {
      await prisma.enrollment.upsert({
        ...
        update: {},
        create: { userId: student.id, courseId: c.id, status: 'ACTIVE' },
      });
    }
    console.log(`\n✔ Student enrolled in ${allCourses.length} courses`);
  }
